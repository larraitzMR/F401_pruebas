...
void RegionEU868GetPhyParam( GetPhyParams_t* getPhy )
{
    switch( getPhy->Attribute )
    {
        ...
        case PHY_DEF_TX_DR:
        {
            getPhy->Param.Value = EU868_DEFAULT_DATARATE;
            break;
        }
        case PHY_DEF_TX_POWER:
        {
            getPhy->Param.Value = EU868_DEFAULT_TX_POWER;
            break;
        }
        case PHY_MAX_PAYLOAD:
        {
            getPhy->Param.Value = MaxPayloadOfDatarateEU868[getPhy->Datarate];
            break;
        }
        case PHY_MAX_PAYLOAD_REPEATER:
        {
            getPhy->Param.Value = MaxPayloadOfDatarateRepeaterEU868[getPhy->Datarate];
            break;
        }
        case PHY_DUTY_CYCLE:
        {
            getPhy->Param.Value = EU868_DUTY_CYCLE_ENABLED;
            break;
        }
        case PHY_MAX_RX_WINDOW:
        {
            getPhy->Param.Value = EU868_MAX_RX_WINDOW;
            break;
        }
        case PHY_RECEIVE_DELAY1:
        {
            getPhy->Param.Value = EU868_RECEIVE_DELAY1;
            break;
        }
        case PHY_RECEIVE_DELAY2:
        {
            getPhy->Param.Value = EU868_RECEIVE_DELAY2;
            break;
        }
        case PHY_JOIN_ACCEPT_DELAY1:
        {
            getPhy->Param.Value = EU868_JOIN_ACCEPT_DELAY1;
            break;
        }
        ...
        {
            getPhy->Param.Value = EU868_MAX_FCNT_GAP;
            break;
        }
        case PHY_ACK_TIMEOUT:
        {
            getPhy->Param.Value = ( EU868_ACKTIMEOUT + randr( -EU868_ACK_TIMEOUT_RND, EU868_ACK_TIMEOUT_RND ) );
            break;
        }
        case PHY_DEF_DR1_OFFSET:
        {
            getPhy->Param.Value = EU868_DEFAULT_RX1_DR_OFFSET;
            break;
        }
        case PHY_DEF_RX2_FREQUENCY:
        {
            getPhy->Param.Value = EU868_RX_WND_2_FREQ;
            break;
        }
        case PHY_DEF_RX2_DR:
        {
            getPhy->Param.Value = EU868_RX_WND_2_DR;
            break;
        }
        ...
        {
            getPhy->Param.Value = EU868_MAX_NB_CHANNELS;
            break;
        }
        case PHY_CHANNELS:
        {
...
void RegionEU868ApplyCFList( ApplyCFListParams_t* applyCFList )
{
    ...
    for( uint8_t i = 0, chanIdx = EU868_NUMB_DEFAULT_CHANNELS; i < 15; i+=3, chanIdx++ )
    {
    ...
    if( adrNext->AdrEnabled == true )
    {
        ...
        else
        {
            ...
            if( adrNext->AdrAckCounter >= ( EU868_ADR_ACK_LIMIT + EU868_ADR_ACK_DELAY ) )
            {
                ...
                {
#if defined( ADRACKREQ_FORWARD_SPEC )
                    if( txPower == EU868_MAX_TX_POWER )
                    {
#endif /* ADRACKREQ_FORWARD_SPEC */
                        if( datarate > EU868_TX_MIN_DATARATE )
                        {
                            datarate--;
                        }
...
bool RegionEU868TxConfig( TxConfigParams_t* txConfig, int8_t* txPower, TimerTime_t* txTimeOnAir )
{
    ...
    int8_t phyTxPower = TxPowersEU868[txConfig->TxPower];
...
bool RegionEU868NextChannel( NextChanParams_t* nextChanParams, uint8_t* channel, TimerTime_t* time )
{
    ...
    if( nextChanParams->AggrTimeOff <= TimerGetElapsedTime( nextChanParams->LastAggrTx ) )
    {
        ...
        nextTxDelay = RegionCommonUpdateBandTimeOff( nextChanParams->DutyCycleEnabled, Bands, EU868_MAX_NB_BANDS );
...
{
    int8_t phyTxPower = TxPowersEU868[continuousWave->TxPower];
